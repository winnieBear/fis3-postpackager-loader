...
module.exports = function(file, resource, ret, opts) {
  ...
  if(typeof opts.js === 'function'){
    pkgJs = opts.js(filepath);
  }else if(typeof opts.js === 'string'){
    pgkJs = opts.js;
  }
  pack(resource.js, pkgJs || 'pkg/${filepath}_aio.js', opts.sourceMap);

  if(typeof opts.css === 'function'){
    pkgCss = opts.css(filepath);
  }else if(typeof opts.js === 'string'){
    pkgCss = opts.css;
  }
  pack(resource.css, pkgCss || 'pkg/${filepath}_aio.css', opts.sourceMap);
  ...
  function pack(list, fileTpl, sourceMap) {
    ...
    //当只有一个文件时也需要进行合并，为了合并这个文件以及它的依赖
